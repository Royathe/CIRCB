...
namespace CIRCBot
{
    ...
    public class Weather
    {
        ...
        {
            Weather weather = Utils.JSONMapper<Weather>(json);

            weather.WeatherDescription = " | ";
            foreach (var weatherJSON in json.weather)
            {
                string desc = Apis.Repositories.WeatherConditions.Get(((object)weatherJSON.id).ToString());
            ...
            if (json.dt != null)
            {
                weather.Date = Utils.UnixTimeStampToDateTime((int)json.dt);
            }
            else
            {
        ...
        public static class HeatIndex
        {
            ...
            public static string ApparentTemperature(string temp, string humidity, string windspeed)
            {
                ...
                apparentTemperature = Math.Round(AT, 2) + TXT.CENTIGRADE;
